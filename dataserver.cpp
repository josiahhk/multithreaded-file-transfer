...
void process_newchannel_request (RequestChannel *_channel)
{
		...
		if(ipc == "q")
			data_channel = new MessageQueueChannel(new_channel_name, RequestChannel::SERVER_SIDE);
		if(ipc == "s")
			data_channel = new SHMChannel(new_channel_name, RequestChannel::SERVER_SIDE);
		pthread_t thread_id;
...
void *handle_process_loop(void *_channel) //maybe make this a requestchannel
{
	
	RequestChannel* channel = (RequestChannel*)_channel;
	for (;;){
		int len = 0;
		//I think it is using SHM's cread, which reads from the SHMBB, which is empty
		//it needs to read from the regular bounded buffer first
...
int main(int argc, char *argv[])
{
	...
	bufsize = atoi(argv[argc-2]);	// modify this to accept buffsize m from the client side
	ipc = argv[argc-1];
	...
	if(ipc =="f")
		...
		control_channel = new MessageQueueChannel("control", RequestChannel::SERVER_SIDE);
	else
		control_channel = new SHMChannel("control", RequestChannel::SERVER_SIDE);
